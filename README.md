...
第一，解释术语`结构`、`架构`、`架构风格`和`架构约束`，为描述Representational State Transfer (REST) 架构风格做准备。
...
第二，解释术语`REST components`、`resource`、`resource representation`、`resource identifier`、`metadata`和`media type`，为描述Representational State Transfer架构风格做准备。
...
第三，描述`Representational State Transfer架构风格`。
